...
ask_relay_method() {
    ...
    echo "  3) Оба метода"
    read -rp "Выбор [1]: " m
    case $m in 2) RELAY_METHOD="socat" ;; 3) RELAY_METHOD="both" ;; *) RELAY_METHOD="iptables" ;; esac
...
setup_iptables() {
    ...
    echo -e "${GREEN}✓ iptables настроен (SSH:${SSH_PORT}, MT:${RU_MT_PORT}, VLESS:${RU_VLESS_PORT}, SOCKS:${RU_SOCKS_PORT})${NC}"
}
...
<p style="font-size:18px;margin:20px 0">Нажмите кнопку ниже, чтобы добавить прокси в Telegram:</p>
<a href="${MT_LINK}" class="btn tg">➕ Добавить в Telegram</a>
<p style="margin-top:20px;font-size:14px;color:#666">
Ссылка ведёт на <strong>${RU_IP}:${RU_MT_PORT}</strong><br>
<code>${MT_LINK}</code>
</p>
</div>
...
<tr><td>MTProto (FakeTLS)</td><td><strong>${RU_MT_PORT}</strong></td><td>${EN_SERVER_IP}:${EN_MT_PORT}</td><td>✅</td></tr>
<tr><td>VLESS (REALITY)</td><td>${RU_VLESS_PORT}</td><td>${EN_SERVER_IP}:${EN_VLESS_PORT}</td><td>✅</td></tr>
...
<span class="lbl">Ссылка для Telegram (RU IP:443):</span>
...
<p><strong>MTProto:</strong> подключайтесь к <strong>${RU_IP}:${RU_MT_PORT}</strong> с secret из EN сервера.</p>
</div>
...
HTML
    ...
    echo -e "${YELLOW}  Ссылка для Telegram: ${RU_IP}:${RU_MT_PORT}${NC}"
}
...
menu() {
    while true; do
        ...
        case $choice in
            1)
                disable_ufw; ask_en_params || { read -rp "Enter..."; continue; }; ask_relay_method; install_deps
                case $RELAY_METHOD in iptables) setup_iptables ;; socat) setup_socat ;; both) setup_iptables; setup_socat ;; esac
            ...
            2)
                [ ! -f "$CONFIG_FILE" ] && { echo -e "${RED}✗ Конфиг не найден${NC}"; } || {
                    source "$CONFIG_FILE"
                    [ "$RELAY_METHOD" = "socat" ] || [ "$RELAY_METHOD" = "both" ] && { systemctl restart relay-vless relay-socks relay-mtproto; sleep 1; for s in relay-vless relay-socks relay-mtproto; do systemctl is-active --quiet $s && echo -e "${GREEN}✓ $s${NC}" || echo -e "${RED}✗ $s${NC}"; done; }
                    [ "$RELAY_METHOD" = "iptables" ] || [ "$RELAY_METHOD" = "both" ] && setup_iptables
                    echo -e "${GREEN}✅ Перезапуск завершён${NC}"
                }
                ;;
            3)
                ask_en_params || { read -rp "Enter..."; continue; }; ask_relay_method
                case $RELAY_METHOD in iptables) setup_iptables ;; socat) setup_socat ;; both) setup_iptables; setup_socat ;; esac
                save_config; generate_html
                echo -e "${GREEN}✅ Конфигурация обновлена${NC}"
                ;;
            4)
                [ -f "$CONFIG_FILE" ] && { source "$CONFIG_FILE"; echo -e "${BLUE}EN сервер:${NC} $EN_SERVER_IP"; echo -e "${BLUE}Метод:${NC}     $RELAY_METHOD"; }
                echo -e "\n${BLUE}═══ iptables NAT ═══${NC}"; iptables -t nat -L -n -v 2>&1 | grep -E "(Chain|DNAT|MASQUERADE)" || echo "Правил нет"
                echo -e "\n${BLUE}═══ socat сервисы ═══${NC}"; for s in relay-vless relay-socks relay-mtproto; do systemctl is-active --quiet $s 2>/dev/null && echo -e "${GREEN}  ✓ $s${NC}" || echo -e "${RED}  ✗ $s${NC}"; done
                ;;
            5)
                [ -f "$CONFIG_FILE" ] && { source "$CONFIG_FILE"; echo -e "${BLUE}EN сервер:${NC}     $EN_SERVER_IP"; echo -e "${BLUE}MTProto:${NC}       $RU_MT_PORT → $EN_SERVER_IP:$EN_MT_PORT"; echo -e "${BLUE}VLESS:${NC}         $RU_VLESS_PORT → $EN_SERVER_IP:$EN_VLESS_PORT"; echo -e "${BLUE}SOCKS:${NC}         $RU_SOCKS_PORT → $EN_SERVER_IP:$EN_SOCKS_PORT"; echo -e "${BLUE}Метод:${NC}         $RELAY_METHOD"; echo -e "${YELLOW}HTML-файл:${NC} $HTML_FILE"; } || echo -e "${RED}✗ Конфигурация не найдена${NC}"
                ;;
            6)
                read -rp "${RED}Удалить relay? (y/N): ${NC}" ans
                [ "$ans" = "y" ] || [ "$ans" = "Y" ] && { systemctl stop relay-vless relay-socks relay-mtproto 2>/dev/null; systemctl disable relay-vless relay-socks relay-mtproto 2>/dev/null; rm -f /etc/systemd/system/relay-*.service "$HTML_FILE" "$CONFIG_FILE"; systemctl daemon-reload; iptables -t nat -F 2>/dev/null; mkdir -p /etc/iptables; iptables-save > /etc/iptables/rules.v4 2>/dev/null; echo -e "${GREEN}✅ Relay удалён${NC}"; }
                ;;
